...
public class SignDocumentTask implements TaskListener {
    ...
    private static Log logger = LogFactory.getLog(SignDocumentTask.class);
    ...
    public void notify(DelegateTask task) {
        ...
        if(listaNodeRefDocumenti.size() == 0) {
            logger.debug("Nessun Documento Selezionato!");
        } else {
            logger.debug("NodeRef Documento Selezionato: " +listaNodeRefDocumenti.get(0));
        }

//        String recipient = (String) task.getVariable(SignDocumentTask.RECIP_PROCESS_VARIABLE);
//
...
//
//        mailAction.setParameterValue(MailActionExecuter.PARAM_SUBJECT, SignDocumentTask.SUBJECT);        
//        mailAction.setParameterValue(MailActionExecuter.PARAM_TO, recipient);
//        mailAction.setParameterValue(MailActionExecuter.PARAM_FROM, SignDocumentTask.FROM_ADDRESS);
